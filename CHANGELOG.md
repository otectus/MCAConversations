...
**What that comes to.** 2,448 beat contracts and 2,058 reply contracts across 555 question nodes and
2,105 buttons; 8,937 lang keys in each of English and Portuguese; 37 profession profiles; 1,549 chat
intents, which is every reply button in the mod. No route in the corpus is uncontracted, every topic
...
**One target is short of the spec.** Section 16 asks for meaningful personality overlays on at least
25% of referenced say pools; this release has 17.7% (418 of 2,367). Every pool the spec *designates*
as a signature beat is covered in all twenty-one personality namespaces — professions, ruptures,
disclosures, commitments and callbacks — so the gap is volume above that tier rather than a hole in
it, and the overlay ledger is empty rather than being used to excuse the difference.
