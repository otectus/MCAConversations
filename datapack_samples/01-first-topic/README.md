...
So copy this same folder into **both** places. If you only install the data half, the conversation
runs correctly and every line renders as a raw translation key (`dialogue.sample_sleep.ordinary/2`).
If you only install the resource half, nothing happens at all. `pack_format` 15 is correct for
Minecraft 1.20.1 in both roles, which is why one folder can serve as both.

```
saves/<world>/datapacks/01-first-topic/     <- data half
resourcepacks/01-first-topic/               <- assets half
```
...
`pack_format` 15 is 1.20.1 for both data packs and resource packs. There is no `filter` and no
`overlays` section: this pack only adds, it never hides anything the mod ships.
