...
| Half | Where it goes |
|---|---|
| `data/sample_ledger/**` | `saves/<world>/datapacks/04-arcs-and-promises/` |
| `assets/sample_ledger/**` | `resourcepacks/04-arcs-and-promises/` |

Copy the whole folder to both. Minecraft serves only `data/` from a datapack and only `assets/` from
a resource pack; the extra directory in each copy is ignored. `pack_format` 15 is 1.20.1 for both.
...
- `greedy` / `crabby` / `confident` → **−1 heart**, tension +4, respect −2, and the line
...
`conversations_personality` never throws and resolves 7.6 ids to their 7.7 successors.
