...
folder and `assets/` from a resource pack folder. **Install the two halves separately:**

```
07-world-and-compat/datapack/     -> saves/<world>/datapacks/07-world-and-compat/
07-world-and-compat/resourcepack/ -> resourcepacks/07-world-and-compat/
```
...
`pack_format` is **48** for the data half and **34** for the resource half — on 1.21.1 the two pack
kinds no longer share a number, which is why the sample ships as two folders with two manifests.
Paths written `data/…` or `assets/…` below are paths inside an installed pack.
...
`pack_format` and a description, nothing else — `48` in the datapack manifest, `34` in the resource
pack one.
