...
```
saves/<world>/datapacks/08-profession-pack/     -> data/sample_chandler/...
resourcepacks/08-profession-pack/               -> assets/sample_chandler/lang/en_us.json
```
...
`pack_format` is **15** (1.20.1) for both.
...
stance: `confident`, `crabby`, `greedy`, `relaxed`, `peaceful` and `upbeat` take it on the chin,
...
- **landed** (`confident`, `crabby`, `greedy`, `relaxed`, `peaceful`, `upbeat`): +1 heart,
...
the world load — down with it. This one never throws and resolves MCA 7.6 ids to their 7.7
successors, so one authored list works on both.
