...
- Two **personality overlay** lang files showing the prefixed-key rule
  (`gloomy.dialogue.…`, `extroverted.dialogue.…`).
...
| Half | In this folder | What is in it | Where it goes |
|---|---|---|---|
| data | `datapack/` | `data/sample_courage/**` | `saves/<world>/datapacks/03-checks-and-dispositions/` |
| assets | `resourcepack/` | `assets/sample_courage/**`, `assets/mca_dialogue_*/**` | `resourcepacks/03-checks-and-dispositions/` |

Copy **each half to its own place**. Minecraft only serves `data/` out of a datapack and only serves
`assets/` out of a resource pack, and on 1.21.1 the two kinds no longer share a `pack_format`, so
each half carries its own `pack.mcmeta`. (Inside a mod jar the split does not exist — one jar
carries both.) Paths written as `data/…` or `assets/…` below are paths inside an installed pack.
...
`datapack/pack.mcmeta` declares `"pack_format": 48` and `resourcepack/pack.mcmeta` declares
`"pack_format": 34` — the two 1.21.1 numbers, which is why there are two of them.
...
"description": "…"}}`. Nothing else. 48 is the 1.21.1 data-pack format, 34 the resource-pack one.
...
of remark land better on an `upbeat` villager and worse on an `anxious` one; the bias is capped at
...
it, not patching it. This sample deliberately touches only `anxious` and `upbeat`.

`anxious` gets `encouragement: -5` and `restraint: +7`; `upbeat` gets `encouragement: +9`. So the
`steady` button is the harder read on an anxious villager and the easy one on an upbeat villager —
which is the checklist item "there is no universally correct button", implemented in the one place
it belongs.

Both ids are ones a villager on 1.21.1 can roll. Profiles are keyed by personality id, so a profile
for `athletic`, `confident` or `peppy` would load fine and simply never apply to anybody: MCA no
longer registers those three, so no generated villager has one.
...
### `assets/mca_dialogue_gloomy/lang/en_us.json` and `assets/mca_dialogue_extroverted/lang/en_us.json`
...
why the `gloomy` overlays here are 2 lines (base `.guard` and `.rebuff` are 2) and the
`extroverted` opener overlay is 3 (base `courage.open.first` is 3).

Both namespaces name a personality a 1.21.1 villager can roll. The mod still ships
`mca_dialogue_confident`, `_peppy` and `_athletic`, and writing overlays into them is still valid —
but MCA no longer registers those three personalities, so on a stock world nobody would ever be
shown them.
