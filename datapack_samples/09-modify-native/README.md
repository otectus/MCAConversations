...
place. Install the two halves separately:

- `09-modify-native/datapack/` -> `saves/<world>/datapacks/09-modify-native/` — Minecraft reads only
  `data/` from here.
- `09-modify-native/resourcepack/` -> `resourcepacks/09-modify-native/` — Minecraft reads only
  `assets/` from here, and you must **enable it in Options -> Resource Packs**.
...
`pack_format` is **48** for the data half and **34** for the resource half. On 1.21.1 those two
numbers have diverged, so one `pack.mcmeta` can no longer serve both roles the way 15 did on 1.20.1
— which is why this sample ships as two folders. Paths written `data/…` or `assets/…` below are
paths inside an installed pack.
...
`{"pack": {"pack_format": 48, "description": "..."}}` in `datapack/`, and
`{"pack": {"pack_format": 34, "description": "..."}}` in `resourcepack/`. Nothing else. Two
manifests, because the 1.21.1 data and resource formats are different numbers.
...
- **It is not tested in a running game.** These files are authored against the shipped schemas, the
  ...
  `net.conczin.mca`). They have not been run in a production instance.
