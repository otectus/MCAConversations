 ...
 * {@code forge.net.mca.*} imports; {@code compat.mca.RealTalkMcaRegistrar} (which does) is only
...
public final class McaBridge {
    ...
    public static void tryRegister() {
        if (!ModList.get().isLoaded("mca")) {
            McaRealTalk.LOGGER.warn("MCA Reborn not present; MCA: Real Talk features disabled.");
            available = false;
            return;
        }
        try {
            dev.otectus.mcarealtalk.compat.mca.RealTalkMcaRegistrar.register();
            available = true;
            McaRealTalk.LOGGER.info("MCA: Real Talk dialogue conditions/actions registered with MCA.");
        } catch (Throwable t) {
            available = false;
            McaRealTalk.LOGGER.error(
                    "Failed to register MCA dialogue extensions; Real Talk features disabled. "
