...
/**
 * Parsed form of the {@code realtalk_say} action JSON:
 * {@code {"phrase": "realtalk.us.firstmet", "vars": ["villager_name"]}}.
 *
...
public record SayDirective(String phrase, List<TemplateVariable> vars) {
    ...
    public static SayDirective fromJson(JsonObject json) {
        if (json == null || !json.has("phrase")) {
            throw new IllegalArgumentException("realtalk_say requires a \"phrase\"");
        }
        String phrase = json.get("phrase").getAsString();
        if (phrase.isBlank()) {
            throw new IllegalArgumentException("realtalk_say \"phrase\" must not be blank");
        }
        ...
        if (json.has("vars")) {
            for (JsonElement e : json.getAsJsonArray("vars")) {
                ...
                vars.add(TemplateVariable.byJsonName(name).orElseThrow(
                        () -> new IllegalArgumentException("realtalk_say unknown var \"" + name + "\"")));
            }
        }
