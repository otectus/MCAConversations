...
/**
 * Parsed form of the {@code conversations_gossip} condition / {@code conversations_gossip_say} action JSON:
            ...
            if (json.has("types")) {
                for (JsonElement e : json.getAsJsonArray("types")) {
                    ...
                    types.add(GossipEventType.byJsonName(name).orElseThrow(
                            () -> new IllegalArgumentException("conversations_gossip unknown type \"" + name + "\"")));
                }
            }
            if (json.has("max_age")) {
