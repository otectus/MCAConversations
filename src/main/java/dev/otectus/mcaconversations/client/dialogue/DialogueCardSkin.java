...
public final class DialogueCardSkin {
    ...
    private static void button(GuiGraphics graphics, DialogueChoiceLayout.Rect rect, float alpha,
                               int v) {
        blitNineSliced(graphics, WIDGETS, 0, v, BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_CORNER,
                rect, alpha);
    }
     ...
     *
     * <p>Written locally rather than through {@code GuiGraphics.blitNineSliced}, which does not exist
     * in this Minecraft version at all; nine explicit blits behave the same and compile everywhere.
     */
    private static void blitNineSliced(GuiGraphics graphics, ResourceLocation sheet,
                                       int u0, int v0, int sourceWidth, int sourceHeight, int corner,
                                       DialogueChoiceLayout.Rect rect, float alpha) {
        List<Patch> patches = patches(u0, v0, sourceWidth, sourceHeight, corner, rect);
        if (patches.isEmpty()) {
            return;
        }
        RenderSystem.enableBlend();
        ...
        try {
            ...
            for (Patch patch : patches) {
                graphics.blit(sheet, patch.x(), patch.y(), patch.width(), patch.height(),
                        patch.u(), patch.v(), patch.uWidth(), patch.vHeight(), 256, 256);
            }
        } finally {
            graphics.setColor(1.0F, 1.0F, 1.0F, 1.0F);
            RenderSystem.disableBlend();
