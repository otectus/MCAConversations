...
public final class DialogueCardPreviewScreen extends Screen {
    ...
    private static String onOff(ForgeConfigSpec.ConfigValue<Boolean> value) {
        try {
    ...
    private static String read(ForgeConfigSpec.ConfigValue<?> value) {
        try {
    ...
    private static <T> void cycle(ForgeConfigSpec.ConfigValue<T> value, List<T> options) {
        try {
    ...
    private static void toggle(ForgeConfigSpec.ConfigValue<Boolean> value) {
        try {
            value.set(!value.get());
        } catch (Throwable ignored) {
            // A preview control must never take the development client down.
