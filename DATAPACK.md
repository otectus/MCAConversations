...
an allow-list that names every authorable group. It is enforced, together with the optional
`kingdom_gate` and `civic_contact` fields below, by one composite gate, `conversation/TopicGate`, on
every entry path a catalogued topic can be reached from: the MCA dialogue screen's answer list
(`mixin/QuestionMixin`, which removes a refused answer before it is ever clickable — MCA's own result
conditions run too late to hide a menu entry), dialogue-screen submissions
(`mixin/InteractionDialogueMessageMixin`), the numbered-choice packet
(`conversation/ChoiceSelectionService`), free-text chat and typed hub shortcuts (`chat/GatePreview`,
`chat/ChatModeDispatcher`), and the dynamic hub's slots (`hub/DynamicHub`). A topic with no catalog
row is not gated by any of this — it is left exactly as MCA's own dialogue JSON offers it.
...
Ultima Kingdoms has no NeoForge 1.21.1 release yet, so on this port the bridge stays inert and every
gated topic stays hidden unless its gate explicitly allows an unknown answer.
