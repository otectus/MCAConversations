...
| `capital.at_war` | boolean | — | Whether the capital is currently at war (only when diplomacy talk enabled) |
...
| `capital.sovereign_is_player` | boolean | — | Whether some player (not necessarily this one) holds the throne |
...
already contracted; everything else is the rule for when that route is the right one.
...
**`slots_used` and `vars_used`** (both optional, informational fields, purely for documentation):
- `slots_used` — array of slot ids from `required_slots` that this scene actually uses in its dialogue lines. The compiler tracks which slots are read to verify this is exhaustive and no scenario is dead code.
- `vars_used` — array of template variable names (e.g., `capital_name`, `season`, `profession_name`) that the scene's say pool expects. Valid values: all vars listed in the *Template variables* section above, plus capital variables (`capital_name`, `sovereign_name`, `sovereign_title`, `heir_name`, `house_name`, `house_words`, `villager_title`, `rival_capital_name`, `ally_capital_name`) and Townstead variables. No enforcement: purely informational for content authors.
